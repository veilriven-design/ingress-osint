...
The demo mode delivers a netvis-quality live experience with realistic
military OSINT signals "as they enter the open domain".
...
# that need them. This allows `pip install -e " .[demo]"` (rich + typer only) to
# successfully start the CLI and run `ingress demo` without pulling in optional
...
MAX_RECENT = 12
SPARK_HISTORY = 24
DEMO_THEATERS = ["Donbas", "Black Sea", "Belgorod", "Kharkiv", "Zaporizhzhia"]

STOP_EVENT = threading.Event()
SIGNAL_QUEUE: Any = None  # will be queue in real run
...
def demo_feeder(stop_evt: threading.Event, q: Any) -> None:
...
def build_header(stats: str) -> Panel:
    ...
    subtitle = Text("  Military OSINT • Signals as they enter the open domains  •  v" + __version__, style="dim")
    return Panel(
...
def build_sources_panel() -> Panel:
    ...
    return Panel(t, title="Top Sources (demo)", border_style="green", box=box.ROUNDED)
...
def build_entities_panel() -> Panel:
    ...
    return Panel(t, title="Mentioned Entities", border_style="yellow", box=box.ROUNDED)
...
def build_footer(rate: float, spark: str) -> Text:
    return Text.assemble(
        ...
        ("   ", ""),
        ("[AGPL-3.0] For authorized research & analysis only. Respect all source ToS.", "dim"),
    )
...
def render_dashboard() -> Layout:
    ...
    stats = f"Signals: {n_sig}   Sources: {n_src}   Events: {len(event_times)}   {int(elapsed)}s   Theaters: {', '.join(random.sample(DEMO_THEATERS, 2))}"
...
def run_demo(run_seconds: float = 0) -> None:
    """Run the beautiful live demo TUI (no external services required)."""
    if not HAS_RICH:
        print("FATAL: rich is required for the Ingress TUI.")
        print("Requires Python >= 3.10. Install with: python3 -m pip install -e '.[demo]'")
    ...
    console.print(Panel.fit(
        "[bold red]INGRESS[/]  •  Military OSINT Megatool (demo)\n"
        "[dim]Real-time view of signals entering the open domain — simulated but realistic.\n"
        "Full provenance, confidence, and verification status shown for every item.[/]",
        border_style="red",
    ))
    console.print("[yellow]This is canned demo data for illustration. Real collectors (RSS, Telegram, ADS-B, etc.) are under development.[/]")
    console.print("[dim]Press 'q' or Ctrl-C to exit. See --help for other options.[/]\n")

    t_demo = threading.Thread(target=demo_feeder, args=(STOP_EVENT, SIGNAL_QUEUE), daemon=True)
    t_demo.start()
    ...
    try:
        ...
        STOP_EVENT.set()
        time.sleep(0.1)
        console.print("\n[bold]Ingress demo session ended.[/]")
        with STATE_LOCK:
...
def run_watch(db_url: str | None = None, run_seconds: float = 0) -> None:
    """Real live TUI pulling from storage (PR7: more than a demo, integrated real data)."""
    if not HAS_RICH:
        print("FATAL: rich is required for the Ingress TUI.")
        raise SystemExit(1)
    ...
    try:
        ensure_schema(db)
        real_artifacts = get_recent_artifacts(30, db)
        ...
        console.print(f"[yellow]Could not load real data from DB ({e}). Falling back to canned demo.[/]")
        for s in DEMO_SIGNALS:
            SIGNAL_QUEUE.put(s)

    console.print(Panel.fit(
        "[bold red]INGRESS WATCH[/]  •  Live view of REAL open-domain military signals\n"
        "[dim]Pulls live from your storage (sqlite/Postgres). Run 'ingress ingest ...' in another terminal to feed it.[/]",
        border_style="red",
    ))
    console.print("[dim]Press 'q' or Ctrl-C to exit. This is a functional real-data TUI (beyond demo).[/]\n")
...
def demo(
    ...
    real: bool = typer.Option(False, "--real", help="Pull from real DB storage instead of canned data (PR7)"),
) -> None:
    """Run the beautiful live demo TUI with realistic military OSINT signals."""
    if real:
        run_watch(db_url=None, run_seconds=run_seconds)
    else:
        run_demo(run_seconds=run_seconds)
...
def watch(
    ...
    """Live TUI watching REAL data from storage (integrated, more than a demo - PR7)."""
...
def watch(
    ...
    """Live TUI watching REAL data from storage (integrated, more than a demo)."""
    run_watch(db_url=db_url, run_seconds=run_seconds)
...
def ingest_rss(
    ...
    db_url: str | None = typer.Option(
        ...
        help="Database URL (sqlite://... or postgresql://...). Defaults to config / sqlite demo DB.",
    ),
...
def ingest_x(
    ...
    Real implementation requires X API access (paid tiers as of 2024).
...
if __name__ == "__main__":  # pragma: no cover
    # Support `python -m ingress.cli demo`
    import sys
